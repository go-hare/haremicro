...
func init() {
	generator.RegisterPlugin(new(haremicro))
}

// haremicro is an implementation of the Go protocol buffer compiler's
// plugin architecture.  It generates bindings for go-haremicro support.
type haremicro struct {
	gen *generator.Generator
}

// Name returns the name of this plugin, "haremicro".
func (g *haremicro) Name() string {
	return "haremicro"
}
...
func (g *haremicro) Init(gen *generator.Generator) {
	g.gen = gen
...
func (g *haremicro) objectNamed(name string) generator.Object {
	g.gen.RecordTypeUse(name)
	return g.gen.ObjectNamed(name)
}

// Given a type name defined in a .proto, return its name as we will print it.
func (g *haremicro) typeName(str string) string {
	return g.gen.TypeName(g.objectNamed(str))
}

// P forwards to g.gen.P.
func (g *haremicro) P(args ...interface{}) { g.gen.P(args...) }

// Generate generates code for the services in the given file.
func (g *haremicro) Generate(file *generator.FileDescriptor) {
...
func (g *haremicro) GenerateImports(file *generator.FileDescriptor, imports map[generator.GoImportPath]generator.GoPackageName) {
	if len(file.FileDescriptorProto.Service) == 0 {
		return
	}
	g.P("import (")
...
var reservedClientName = map[string]bool{
	// TODO: do we need any in go-haremicro?
}
...
func (g *haremicro) generateService(file *generator.FileDescriptor, service *pb.ServiceDescriptorProto, index int) {
...
func (g *haremicro) generateEndpoint(servName string, method *pb.MethodDescriptorProto) {
	if method.Options == nil || !proto.HasExtension(method.Options, options.E_Http) {
		return
	}
	// http rules
...
func (g *haremicro) generateClientSignature(servName string, method *pb.MethodDescriptorProto) string {
...
func (g *haremicro) generateClientMethod(reqServ, servName, serviceDescVar string, method *pb.MethodDescriptorProto, descExpr string) {
...
func (g *haremicro) generateServerSignature(servName string, method *pb.MethodDescriptorProto) string {
	origMethName := method.GetName()
	methName := generator.CamelCase(origMethName)
	if reservedClientName[methName] {
		methName += "_"
...
func (g *haremicro) generateServerMethod(servName string, method *pb.MethodDescriptorProto) string {
	methName := generator.CamelCase(method.GetName())
