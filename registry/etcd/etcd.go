...
func (e *etcdRegistry) registerNode(s *registry.Service, node *registry.Node, opts ...registry.RegisterOption) error {
	...
	if logger.V(logger.TraceLevel, logger.DefaultLogger) {
		logger.Tracef("Registering %s id %s [%s][%s] with lease %v and leaseID %v and ttl %v",
			service.Name, node.Id, nodePath(service.Name, node.Id), encode(service), lgr, lgr.ID, options.TTL)
	}
