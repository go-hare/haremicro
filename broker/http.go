...
func (h *httpBroker) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	if req.Method != "POST" {
		err := merrors.BadRequest("hare.micro.broker", "Method not allowed")
	...
	if err != nil {
		errr := merrors.InternalServerError("hare.micro.broker", "Error reading request body: %v", err)
		w.WriteHeader(500)
	...
	if err = h.opts.Codec.Unmarshal(b, &m); err != nil {
		errr := merrors.InternalServerError("hare.micro.broker", "Error parsing request body: %v", err)
		w.WriteHeader(500)
		w.Write([]byte(errr.Error()))
		return
	}
	...
	if len(topic) == 0 {
		errr := merrors.InternalServerError("hare.micro.broker", "Topic not found")
		w.WriteHeader(500)
		w.Write([]byte(errr.Error()))
		return
	}
...
func (h *httpBroker) Init(opts ...Option) error {
	...
	if len(h.id) == 0 {
		h.id = "hare.micro.http.broker-" + uuid.New().String()
	}
