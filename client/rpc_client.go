...
import (
	...
	"github.com/kong11213613/haremicro/common/util/net"
	"github.com/kong11213613/haremicro/common/util/pool"
...
func (r *rpcClient) call(ctx context.Context, node *registry.Node, req Request, resp interface{}, opts CallOptions) error {
	...
	if cf == nil {
		...
		if err != nil {
			return errors.InternalServerError("hare.micro.client", err.Error())
		}
	}
	...
	if err != nil {
		return errors.InternalServerError("hare.micro.client", "connection error: %v", err)
	}
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				ch <- errors.InternalServerError("hare.micro.client", "panic recovered: %v", r)
			}
		}()
	...
	case err := <-ch:
		return err
	case <-ctx.Done():
		grr = errors.Timeout("hare.micro.client", fmt.Sprintf("%v", ctx.Err()))
	}
...
func (r *rpcClient) stream(ctx context.Context, node *registry.Node, req Request, opts CallOptions) (Stream, error) {
	...
	if cf == nil {
		...
		cf, err = r.newCodec(req.ContentType())
		if err != nil {
			return nil, errors.InternalServerError("hare.micro.client", err.Error())
		}
	}
	...
	if err != nil {
		return nil, errors.InternalServerError("hare.micro.client", "connection error: %v", err)
	}
...
func (r *rpcClient) next(request Request, opts CallOptions) (selector.Next, error) {
	...
	if err != nil {
		if err == selector.ErrNotFound {
			return nil, errors.InternalServerError("hare.micro.client", "service %s: %s", service, err.Error())
		}
		return nil, errors.InternalServerError("hare.micro.client", "error selecting %s node: %s", service, err.Error())
	}
...
func (r *rpcClient) Call(ctx context.Context, request Request, response interface{}, opts ...CallOption) error {
	...
	select {
	case <-ctx.Done():
		return errors.Timeout("hare.micro.client", fmt.Sprintf("%v", ctx.Err()))
	default:
	}
	...
	// return errors.New("hare.micro.client", "request timeout", 408)
	call := func(i int) error {
		// call backoff first. Someone may want an initial start delay
		t, err := callOpts.Backoff(ctx, request, i)
		if err != nil {
			return errors.InternalServerError("hare.micro.client", "backoff error: %v", err.Error())
		}
		...
		if err != nil {
			if err == selector.ErrNotFound {
				return errors.InternalServerError("hare.micro.client", "service %s: %s", service, err.Error())
			}
			return errors.InternalServerError("hare.micro.client", "error getting next %s node: %s", service, err.Error())
		}
	...
	for i := 0; i <= retries; i++ {
		...
		select {
		case <-ctx.Done():
			return errors.Timeout("hare.micro.client", fmt.Sprintf("call timeout: %v", ctx.Err()))
		case err := <-ch:
...
func (r *rpcClient) Stream(ctx context.Context, request Request, opts ...CallOption) (Stream, error) {
	...
	select {
	case <-ctx.Done():
		return nil, errors.Timeout("hare.micro.client", fmt.Sprintf("%v", ctx.Err()))
	default:
	}
	...
	call := func(i int) (Stream, error) {
		...
		t, err := callOpts.Backoff(ctx, request, i)
		if err != nil {
			return nil, errors.InternalServerError("hare.micro.client", "backoff error: %v", err.Error())
		}
		...
		if err != nil {
			...
			return nil, errors.InternalServerError("hare.micro.client", "error getting next %s node: %s", service, err.Error())
		}
	...
	for i := 0; i <= retries; i++ {
		...
		select {
		case <-ctx.Done():
			return nil, errors.Timeout("hare.micro.client", fmt.Sprintf("call timeout: %v", ctx.Err()))
		case rsp := <-ch:
			// if the call succeeded lets bail early
			if rsp.err == nil {
